...
export default function DisplayMyPurchasedNft(){
    const [items, setItems] = useState<ItemInfo[]>([])
    const [currentAccount, setCurrentAccount] = useState<string | undefined>()
    const [signer, setSigner] = useState<ethers.JsonRpcSigner>()
    const [show, setShow] = useState<boolean>(false)
    const[content, setContent] = useState<string>("DisplayMyPurchasedNft")
    function handle(){
        setShow(!show)
        if(!show){
            setContent("HideMyPurchasedNft")
        }else{
            setContent("DisplayMyPurchasedNft")
        }      
    }
    useEffect( () => {
      ...
      // market.connect(signer).fetchMyPurchasedItems().then((result)=>{
      //   setItems(result)
      //   console.log("fetchMyPurchasedItems length:", result.length
      // }).catch((e)=>console.log(e))

      market.fetchMyPurchasedItems({from:currentAccount}).then((result)=>{
        setItems(result)
        console.log("fetchMyPurchasedItems length:", result.length)
      }).catch((e)=>console.log(e))

      if(!items) return
      console.log("fetchMyPurchasedItems length:", items.length)
      ...
      <Grid templateColumns='repeat(3, 1fr)' gap={0} w='50%'>
        ...
        : items.map((item:any) => (
                <GridItem key={item.id} >
                  ...
                  <Text fontSize='xl' px={2} py={2}>owner: mine</Text>
              </GridItem>
              ))
        :<Box></Box>}
      </Grid>
